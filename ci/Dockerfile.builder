...
# qnodecoin_hash
RUN git clone https://github.com/qnodecoinpay/qnodecoin_hash
RUN cd qnodecoin_hash && python3 setup.py install
...
RUN groupadd -g ${GROUP_ID} qnodecoin
RUN useradd -u ${USER_ID} -g qnodecoin -s /bin/bash -m -d /qnodecoin qnodecoin
...
RUN mkdir /qnodecoin-src && \
  mkdir -p /cache/ccache && \
  mkdir /cache/depends && \
  mkdir /cache/sdk-sources && \
  chown $USER_ID:$GROUP_ID /qnodecoin-src && \
  chown $USER_ID:$GROUP_ID /cache && \
  chown $USER_ID:$GROUP_ID /cache -R
WORKDIR /qnodecoin-src
