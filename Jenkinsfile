...
// which are part of the QnodeCoin repo will allow modification to the Jenkinsfile.
...
for(int i = 0; i < targets.size(); i++) {
  ...
  tasks["${target}"] = {
    node {
      ...
      withEnv(env) {
        def builderImageName="qnodecoin-builder-${target}"
        ...
        builderImage.inside("-t") {
          ...
          sh "cp -ra ${pwd}/. /qnodecoin-src/"

          // restore cache
          def hasCache = false
          try {
            copyArtifacts(projectName: "qnodecoinpay-qnodecoin/${BRANCH_NAME}", optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz")
          } catch (Exception e) {
          }
          if (fileExists("ci-cache-${target}.tar.gz")) {
            hasCache = true
            echo "Using cache from qnodecoinpay-qnodecoin/${BRANCH_NAME}"
          } else {
            try {
              copyArtifacts(projectName: 'qnodecoinpay-qnodecoin/develop', optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz");
            } catch (Exception e) {
            }
            if (fileExists("ci-cache-${target}.tar.gz")) {
              hasCache = true
              echo "Using cache from qnodecoinpay-qnodecoin/develop"
            }
          }

          if (hasCache) {
            sh "cd /qnodecoin-src && tar xzf ${pwd}/ci-cache-${target}.tar.gz"
          } else {
            sh "mkdir -p /qnodecoin-src/ci-cache-${target}"
          }

          stage("${target}/depends") {
            sh 'cd /qnodecoin-src && ./ci/build_depends.sh'
          }
          stage("${target}/build") {
            sh 'cd /qnodecoin-src && ./ci/build_src.sh'
          }
          stage("${target}/test") {
            sh 'cd /qnodecoin-src && ./ci/test_unittests.sh'
          }
          stage("${target}/test") {
            sh 'cd /qnodecoin-src && ./ci/test_integrationtests.sh'
          }

          // archive cache and copy it into the jenkins workspace
          sh "cd /qnodecoin-src && tar czfv ci-cache-${target}.tar.gz ci-cache-${target} && cp ci-cache-${target}.tar.gz ${pwd}/"
        }
