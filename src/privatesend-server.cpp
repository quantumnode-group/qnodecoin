...
void CPrivateSendServer::ProcessMessage(CNode* pfrom, const std::string& strCommand, CDataStream& vRecv, CConnman& connman)
{
    ...
    if (fLiteMode) return; // ignore all QnodeCoin related functionality
...
/*
    ...
    allow endless transaction that would bloat QnodeCoin and make it unusable. To
...
{
    if (fLiteMode) return;        // disable all QnodeCoin specific functionality
