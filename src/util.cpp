...
const char * const BITCOIN_CONF_FILENAME = "qnodecoin.conf";
const char * const BITCOIN_PID_FILENAME = "qnodecoind.pid";
    ...
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL || *cacheCounter != logAcceptCategoryCacheCounter.load())
        {
            ...
            if (mapMultiArgs.count("-debug")) {
                ...
                // "qnodecoin" is a composite category enabling all QnodeCoin-related debug output
                if(ptrCategory->count(std::string("qnodecoin"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\QnodeCoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\QnodeCoin
    // Mac: ~/Library/Application Support/QnodeCoin
    // Unix: ~/.qnodecoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "QnodeCoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/QnodeCoin";
#else
    // Unix
    return pathRet / ".qnodecoin";
#endif
#endif
}
...
void ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
        // Create empty qnodecoin.conf if it does not excist
...

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override qnodecoin.conf
